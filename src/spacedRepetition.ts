 ...
 */
export function calculateNextReview(progress: WordProgress, quality: number): WordProgress {
  const now = Date.now();
 ...
 */
export function getDueWords(progressData: WordProgress[], currentTime: number = Date.now()): WordProgress[] {
  return progressData.filter((p) => p.nextReview <= currentTime).sort((a, b) => a.nextReview - b.nextReview);
}
